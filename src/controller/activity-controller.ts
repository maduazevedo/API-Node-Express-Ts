...
import { createValidation, idValidation } from "../validations/activities-validation";
import {getActivitiesTypes, getByPaginatedFilterTypeAndOrder, getAllByFilterTypeAndOrder, getActivitiesUserCreator, getActivitiesUserCreatorAll, getActivitiesUserParticipantAll, createActivity, getActivitiesUserParticipant, getParticipantsByActivities, updateActivities, concludeActivities, deleteActivities} from "../service/activity-service";
import { approveParticipants, createSubscriptionInActivity, doCheckin, doUnsubscribe } from "../service/activity-participants-service";
...
export function activityController (server: Express){
    ...
    router.get('/', async(req, res) =>{
        try {
            ...
            const response = await getByPaginatedFilterTypeAndOrder(userId, pageInt, pageSizeInt, typeIdStr, orderByStr, orderStr);
            res.status(200).send(response);
        } catch (error) {
            if (error instanceof ServerError) {
                res.status(500).send({ error: error.message });
    ...
    router.post('/new', upload.single("image"), validatorRequestBody(createValidation), async(req: Request, res) =>{
        ...
        try{
            ...
            const response = await createActivity(creatorId, title, description, type, address, imgUrl, isPrivate, scheduledDate)
            res.status(200).send(response);
        }catch(error){
            if (error instanceof ServerError){
    ...
    router.put('/:id/update', upload.single("image"), async (req: Request, res)=>{
        try{
            const userId = res.userId as string
            const { id } = req.params;
            const { title, description, type, address, scheduledDate, isPrivate} = req.body;
            ...
            const response = await updateActivities(userId, id, title, description, type, address, imgUrl, isPrivate, scheduledDate)
            res.status(200).send(response);
        }catch(error){
            if (error instanceof ServerError){
                res.status(error.statusCode).send({error: error.message})
