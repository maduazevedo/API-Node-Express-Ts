...
export async function findByPaginatedFilterTypeAndOrder( userId:string, page: number, pageSize: number, typeId?: string, orderBy?: string,  order?: string) {

    const skip = (page - 1) * pageSize;

    const userTypes = await auxFunction(userId);
    ...
    if (typeId) {
        whereClause.type = typeId;
    } else if (userTypes.length > 0) {
        whereClause.OR = [{ type: { in: userTypes } }];
    }


    const totalActivities = await prisma.activities.count({ where: whereClause });
...
export async function findAllByFilterTypeAndOrder(userId: string, typeId?: string, orderBy?: string, order?: 'asc' | 'desc') {
...
async function auxFunction(userId: string) {
    const preferences = await prisma.preferences.findMany({
        where: { userId },
        select: { typeId: true }
    });

    return preferences.map(pref => pref.typeId);
}
...
export async function getActivityTypeById(typeId: string[]) {
    const type =  await prisma.activityTypes.findMany({
        where:{
            id: {
                in: typeId
            }
        ...
        return type.length === typeId.length;
};
...
export async function findActivitiesUserParticipantAll(userId: string) {
    ...
    return activities.map(({ activitysId, approved, ...rest }) => ({
        ...activitysId, 
...
export async function findParticipantsByActivity(activityId: string) {
    ...
    return participants.map(({ usersId, approved, ...rest }) => ({
        ...rest,
        name: usersId.name,
...
export async function saveActivity(creatorId: string, title: string, description: string, type: string, image: string, scheduledDate: Date, createdAt: Date, Isprivate: boolean, confirmationCode: string) {
...
export async function updateActivity( idActivity: string, title: string, description: string, type: string, image: string, scheduledDate: Date, isPrivate: boolean){
