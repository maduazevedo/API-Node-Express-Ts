...
import { createAddress, updateAddress } from "../repository/activity-address-repository";
import { findActivityTypes, findByPaginatedFilterTypeAndOrder, findAllByFilterTypeAndOrder, findActivitiesUserCreator, findActivitiesUserCreatorAll, findActivitiesUserParticipant, findActivitiesUserParticipantAll, deleteActivity, existsActivity, existsTypeActivity, hasConcluded, isTheCreatorAct, updateActivity, findParticipantsByActivity, saveActivity, conlcudeActivity } from "../repository/activity-repository";
import { verifyAddress } from "./activity-address-service";
...
// 2. GET/ACTIVITIES
export async function getByPaginatedFilterTypeAndOrder( userId: string, page: number, pageSize: number, typeId?: string, orderBy?: string, order?: 'asc' | 'desc') {

    console.log(userId, page, pageSize, typeId, )
    const result = await findByPaginatedFilterTypeAndOrder(userId, page, pageSize, typeId, orderBy, order);


        const response = result.activities.map(activity => ({
...
export async function getAllByFilterTypeAndOrder( userId: string, typeId?: string, orderBy?: string, order?: 'asc' | 'desc') {
        ...
        const result = await findAllByFilterTypeAndOrder(userId, typeId, orderBy, order);
...
export async function getActivitiesUserCreatorAll(userId: string){
    ...
    const response = result.map(activity => ({
        id: activity.id,
        title: activity.title,
        description: activity.description,
        type: activity.type,
...
export async function createActivity(idCreator: string, title: string, description: string, type: string,  address: string, image: string, isPrivate: string, scheduledDate: string) {
    ...
    try {
        ...
        const activity = await saveActivity( idCreator, title, description, type, image, scheduledDateRaw, createdAt, isPrivateRaw, confirmationCode);
...
export async function updateActivities(idCreator: string, idActivity: string,  title: string, description: string, type: string,  address: string, image: string, isPrivate: string, scheduledDate: string) {
    ...
    try {
        ...
        const activity = await updateActivity( idActivity, title, description, type, image, scheduledDateRaw, isPrivateRaw);
